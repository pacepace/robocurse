...
Create GUI tests for profile schedule dialog and button.

## Objective

Create unit tests for the profile schedule GUI components including the dialog, button state updates, and XAML loading.

## Success Criteria

- [ ] XAML loading tests pass
- [ ] Dialog function exists and is exported
- [ ] Button state update function tests pass
- [ ] Control registration tests verify new controls
- [ ] Integration with profile selection is tested

## Research

### GUI Test Patterns
- Look for existing GUI tests in `tests\Unit\Gui*.Tests.ps1`
- Test XAML loading: `Get-XamlResource` function
- Test control existence without full GUI initialization

### Test Approach
GUI tests typically:
1. Load XAML resources and verify parsing
2. Verify functions exist and are exported
3. Mock dependencies (controls, config)
4. Test logic without actually showing dialogs

## Implementation

Create `tests\Unit\GuiProfileSchedule.Tests.ps1`:
...
Describe "GUI Profile Schedule" {
    Context "XAML Resources" {
        It "Should load ProfileScheduleDialog.xaml" {
            ...
            $xaml | Should -Match 'ProfileScheduleDialog'
        }
    ...
    Context "Button State Function" {
        InModuleScope 'Robocurse' {
            ...
            It "Should show checkmark when schedule enabled" {
                ...
                $script:Controls.btnProfileSchedule.Content | Should -Match "✓"
            ...
            It "Should not show checkmark when schedule disabled" {
                ...
                $script:Controls.btnProfileSchedule.Content | Should -Be "Schedule"
                $script:Controls.btnProfileSchedule.Content | Should -Not -Match "✓"
            }
    ...
    Context "MainWindow XAML Integration" {
        ...
        It "Schedule button should be near Validate button" {
            ...
            # Both buttons should be in the same general area (within 50 lines of each other)
...
- Task 01 (Schema) - for Schedule property in profile
- Task 02 (Core) - for Get-ProfileScheduledTask function
- Task 04 (Dialog) - for ProfileScheduleDialog.xaml and Show-ProfileScheduleDialog
- Task 05 (Button) - for MainWindow changes and button state function

## Files to Create

- `tests\Unit\GuiProfileSchedule.Tests.ps1` (new file)

## Verification

```powershell
# Run GUI profile schedule tests
Invoke-Pester -Path tests\Unit\GuiProfileSchedule.Tests.ps1 -Output Detailed

# Run all GUI tests
Invoke-Pester -Path tests\Unit\Gui*.Tests.ps1 -Output Detailed
