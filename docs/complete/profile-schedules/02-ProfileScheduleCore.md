...
Create core profile scheduling functions in `ProfileSchedule.ps1`.

## Objective

Create a new PowerShell module file `ProfileSchedule.ps1` with functions to manage Windows Task Scheduler tasks for individual profile schedules.

## Success Criteria

- [ ] New file created: `src\Robocurse\Public\ProfileSchedule.ps1`
- [ ] All core functions implemented with proper error handling
- [ ] Functions follow existing patterns from `SnapshotSchedule.ps1`
- [ ] Task naming convention: `Robocurse-Profile-{ProfileName}`
- [ ] Parameter validation prevents command injection
- [ ] OperationResult return pattern used consistently

## Research

### Task Scheduler Pattern Reference
- `src\Robocurse\Public\SnapshotSchedule.ps1:4` - Task prefix pattern: `$script:SnapshotTaskPrefix = "Robocurse-Snapshot-"`
- `src\Robocurse\Public\SnapshotSchedule.ps1:55-155` - `New-SnapshotScheduledTask` function structure
- `src\Robocurse\Public\SnapshotSchedule.ps1:235-274` - `Remove-SnapshotScheduledTask` function
- `src\Robocurse\Public\SnapshotSchedule.ps1:276-308` - `Get-SnapshotScheduledTasks` function
- `src\Robocurse\Public\SnapshotSchedule.ps1:310-381` - `Sync-SnapshotSchedules` function

### Security Patterns
- `src\Robocurse\Public\SnapshotSchedule.ps1:6-11` - Safe parameter patterns
- `src\Robocurse\Public\SnapshotSchedule.ps1:12-53` - `Test-SafeScheduleParameter` function

### Trigger Types
- Hourly: `New-ScheduledTaskTrigger -Once -At $Time -RepetitionInterval (New-TimeSpan -Hours $Interval)`
- Daily: `New-ScheduledTaskTrigger -Daily -At $Time`
- Weekly: `New-ScheduledTaskTrigger -Weekly -DaysOfWeek $Day -At $Time`
- Monthly: Requires `MSFT_TaskTrigger` CIM class (more complex)

## Implementation

Create `src\Robocurse\Public\ProfileSchedule.ps1`:
...
See `03-ProfileScheduleTests.md` for complete test suite.

## Files to Modify

- Create `src\Robocurse\Public\ProfileSchedule.ps1` (new file)
- `src\Robocurse\Robocurse.psd1` - Add to FunctionsToExport list
...
Get-Command -Module Robocurse | Where-Object Name -like '*ProfileSchedule*'
...
Get-ProfileScheduledTask -ProfileName "TestProfile"
