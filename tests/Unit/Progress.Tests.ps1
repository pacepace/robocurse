...
InModuleScope 'Robocurse' {
    Describe "Progress" {
        ...
        Context "ChunksWarning Status Tracking" {
            It "Should include ChunksWarning in status output" {
                $status = Get-OrchestrationStatus

                $status.PSObject.Properties.Name | Should -Contain 'ChunksWarning'
            }

            It "Should return 0 ChunksWarning when no warnings" {
                Initialize-OrchestrationState

                $status = Get-OrchestrationStatus

                $status.ChunksWarning | Should -Be 0
            }

            It "Should return correct ChunksWarning count" {
                Initialize-OrchestrationState

                # Add some warning chunks
                1..3 | ForEach-Object {
                ...
                $status.ChunksWarning | Should -Be 3
            }

            It "Should track warnings independently of failures" {
                Initialize-OrchestrationState
                ...
                # Add failed chunks
                1..5 | ForEach-Object {
                    $chunk = [PSCustomObject]@{ ChunkId = 100 + $_ }
                    $script:OrchestrationState.FailedChunks.Enqueue($chunk)
                }

                $status = Get-OrchestrationStatus

                $status.ChunksWarning | Should -Be 2
                $status.ChunksFailed | Should -Be 5
            }

            It "Should return idle state ChunksWarning as 0" {
                $script:OrchestrationState = $null
        ...
        Context "Get-OrchestrationStatus FilesSkipped" {
            It "Returns FilesSkipped property" {
                $status = Get-OrchestrationStatus

                $status.PSObject.Properties.Name | Should -Contain 'FilesSkipped'
            }

            It "FilesSkipped reflects OrchestrationState value" {
                Initialize-OrchestrationState

                $script:OrchestrationState.AddFilesSkipped(42)

                $status = Get-OrchestrationStatus

                $status.FilesSkipped | Should -Be 42
            }

            It "FilesSkipped is 0 when no files skipped" {
                Initialize-OrchestrationState
