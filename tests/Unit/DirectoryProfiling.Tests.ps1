...
InModuleScope 'Robocurse' {
    Describe "Directory Profiling" {
        Context "ConvertFrom-RobocopyListOutput" {
            ...
            It "Should increment State.ScanProgress for each directory found" {
                # Create mock state with IncrementScanProgress method
                $mockState = [PSCustomObject]@{ CallCount = 0 }
                $mockState | Add-Member -MemberType ScriptMethod -Name 'IncrementScanProgress' -Value {
                    $this.CallCount++
                    return $this.CallCount
                }

                $output = @(
                    "  New Dir          3    C:\Test\Dir1\",
                    "  New Dir          2    C:\Test\Dir2\",
                    "  New Dir          5    C:\Test\Dir3\",
                    "    New File           1024    file.txt"
                )

                $result = ConvertFrom-RobocopyListOutput -Output $output -State $mockState

                $result.DirCount | Should -Be 3
                $mockState.CallCount | Should -Be 3
            }
        }
