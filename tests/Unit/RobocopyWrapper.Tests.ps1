...
Describe "Robocopy Wrapper" {
    ...
    Context "Get-RobocopyExitMeaning" {
        ...
        It "Should interpret exit code 8 as warning (copy errors - robocopy already retried)" {
            # Exit code 8 means some files couldn't be copied, but robocopy already retried
            # per its /R:n setting. We treat this as a warning, not a failure requiring chunk retry.
        ...
        It "Should handle combined exit codes (9 = files copied + copy errors)" {
            # Exit code 9 = 1 + 8: files copied but some couldn't be copied
            # Copy errors (8) are now Warning severity since robocopy already retried
...
InModuleScope 'Robocurse' {
    ...
    Context "New-FailedFilesSummary" {
        BeforeEach {
            Mock Write-RobocurseLog { }
        }

        It "Returns null when no chunk logs exist" {
        ...
        It "Returns null when no error entries in logs" {
            $testLogPath = Join-Path $TestDrive "Logs"
            $testDate = "2025-12-21"
            $datePath = Join-Path $testLogPath $testDate
            $jobsPath = Join-Path $datePath "Jobs"
            ...
            # Create a chunk log with no errors
            $logContent = @(
                "ROBOCOPY     ::     Robust File Copy for Windows"
                "Source : C:\Source\"
                "Dest : D:\Dest\"
                "    New File         1234  file1.txt"
                "    New File         5678  file2.txt"
            )
        ...
        It "Creates FailedFiles.txt with error entries" {
            $testLogPath = Join-Path $TestDrive "Logs"
            $testDate = "2025-12-21"
            $datePath = Join-Path $testLogPath $testDate
            $jobsPath = Join-Path $datePath "Jobs"
            ...
            # Create a chunk log with error lines
            $logContent = @(
                "ROBOCOPY     ::     Robust File Copy for Windows"
                "Source : C:\Source\"
                "Dest : D:\Dest\"
                "    New File         1234  file1.txt"
                "ERROR 32 (0x00000020) Copying File C:\Source\locked.txt"
        ...
        It "Parses ERROR lines correctly" {
            ...
            $logContent = @(
                "ERROR 32 (0x00000020) Copying File C:\Source\important_file.txt"
            )
            $logContent | Out-File -FilePath (Join-Path $jobsPath "Chunk_001.log") -Encoding UTF8
            ...
            $content | Should -Match "ERROR 32"
            $content | Should -Match "important_file.txt"
        }
        ...
        It "Includes file paths in summary" {
            ...
            $logContent = @(
                "ERROR 5 (0x00000005) Copying File C:\Source\Documents\report.docx"
            )
            $logContent | Out-File -FilePath (Join-Path $jobsPath "Chunk_001.log") -Encoding UTF8
        ...
        It "Returns path to created summary file" {
            $testLogPath = Join-Path $TestDrive "Logs"
            $testDate = "2025-12-21"
            $datePath = Join-Path $testLogPath $testDate
            $jobsPath = Join-Path $datePath "Jobs"
            ...
            $logContent = @(
                "ERROR 32 (0x00000020) Copying File C:\Source\file.txt"
            )
            $logContent | Out-File -FilePath (Join-Path $jobsPath "Chunk_001.log") -Encoding UTF8
        ...
        It "Handles multiple chunk logs" {
            $testLogPath = Join-Path $TestDrive "Logs"
            $testDate = "2025-12-21"
            $datePath = Join-Path $testLogPath $testDate
            $jobsPath = Join-Path $datePath "Jobs"
            ...
            # Create multiple chunk logs with errors
            ...
            $content | Should -Match "file3.txt"
        }

        It "Includes chunk headers in output" {
            $testLogPath = Join-Path $TestDrive "Logs"
            $testDate = "2025-12-21"
            $datePath = Join-Path $testLogPath $testDate
            $jobsPath = Join-Path $datePath "Jobs"
